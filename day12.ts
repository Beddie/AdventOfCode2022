...
async function day12(part:number, print: boolean) {
    let answer = 0;
    let endPosition = [];
    ...
    function GridPoint(x, y, hillHeight: number) {
    ...
    function search(startnum: number[], endnum: number[]) {
      init(startnum);
...
function drawGrid(hills: number[][], gridpoints: any[]) {
    let grid = ""
    ...
    console.log(grid)
    writeFileSync(puzzleOutputPath, grid)
}
